...
* Grosse, heterogene Netzwerke mit vielen verschiedenen Admins.
...
** Nutzen Versionsnummern nur, um Aussagen über Verletzlichkeiten zu
   machen. (Und das führt ausserdem gerne noch zu False Positives.)
...
** Betriebssystem-Erkennung eher grob, basiert nur auf
   Netzwerkpaket-Eigenschaften (z.B. TTL und TCP Fenstergrösse), nicht
   Inhalten.
...
Aber es geht genauer: Die Version verrät uns, dass es sich um ein
...
An dieser Stelle wird klar, dass sich diese Art von Informationen nicht
...
* Redis läuft in einem Container (Docker, etc.) weil die Prozess-ID `1`
...
* ob der Admin regelmässig Sicherheitsaktualisierungen einspielt; und
